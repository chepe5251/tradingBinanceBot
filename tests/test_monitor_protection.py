...
import pytest
from bot.monitor.protection import ensure_monitor_protections, ensure_orphan_protections
...
def test_monitor_protection_returns_none_when_position_not_visible() -> None:
    ...
    with patch("bot.monitor.protection.time.time", side_effect=[0.0, 9.0]):
...
def test_orphan_protection_submits_emergency_close_after_repeated_failures() -> None:
    ...
    with patch("bot.monitor.protection.time.sleep", return_value=None):
